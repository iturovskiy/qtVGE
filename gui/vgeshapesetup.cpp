                                                   ...
                                                   _colorButton(new ColorButton(colorTo844(color)))
{
    ...
    _gridLayout->addWidget(_colorLabel, 6, 1);
    _gridLayout->addWidget(_colorButton, 6, 2);
    ...
    _gridLayout->addWidget(_nameLabel, 1, 1);
    _gridLayout->addWidget(_nameLE, 1, 2);
...
void VGEShapeSetUp::init(QPointF point) {
    ...
    _firstCoordYSpinBox = new QDoubleSpinBox(this);
    ...
    _nameLabel->setText("Name:");
    _nameLE->setText(_name);
    _gridLayout->addWidget(_nameLabel, 5, 1);
    _gridLayout->addWidget(_nameLE, 5, 2);
...
void VGEShapeSetUp::save() {
    ...
    if (_firstCoordXSpinBox && _firstCoordYSpinBox) {
        first = QPointF(float(_firstCoordXSpinBox->value()), float(_firstCoordYSpinBox->value()));
    }
    if (_lastCoordYSpinBox && _lastCoordXSpinBox){
        last = QPointF(float(_lastCoordXSpinBox->value()), float(_lastCoordYSpinBox->value()));
    }
    hide();
    emit updateShape(_nameLE->text(), _color, _coefficientSpinBox->value(), first, last,
